...
from TeamXMusic.utils.database import get_model_settings
...
async def api_stats(client, message: Message):
    ...
    try:
        if not AI_ENDPOINT or not AI_KEY:
            await handle_flood_wait(message.reply_text, "❌ Not using TeamX API endpoint please contact @Sid_Elite.")
            return
